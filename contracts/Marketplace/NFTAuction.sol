...
contract NFTAuction is Context, ReentrancyGuard {
    ...
    constructor(
        ...
        require(address(_crypto4AllNft) != address(0), "NFTAuction: Invalid NFT");
    ...
    function placeBid(uint256 _tokenId) external payable nonReentrant whenNotPaused {
        // require(address(_msgSender()).isContract() == false, "NFTAuction.placeBid: No contracts permitted");
    ...
    function resultAuction(uint256 _tokenId) external nonReentrant {
        ...
        require(crypto4AllNft.isApproved(_tokenId, address(this)), "NFTAuction.resultAuction: auction not approved");
        ...
        (bool creatorTransferSuccess,) = crypto4AllNft.ownerOf(_tokenId).call{value : winningBid.sub(platformFeeInETH)}("");
        require(creatorTransferSuccess, "NFTAuction.resultAuction: Failed to send the designer their royalties");

        // Transfer the token to the winner
        crypto4AllNft.safeTransferFrom(crypto4AllNft.ownerOf(_tokenId), winner, _tokenId);
