...
contract NFTMarketplace is Context, ReentrancyGuard {
    ...
    constructor(
        ...
        require(address(_crypto4AllNFT) != address(0), "NFTMarketplace: Invalid NFT");
    ...
    function createOffer(
        ...
        require(crypto4AllNFT.exists(_tokenId), "NFTMarketplace.createOffer: TokenID does not exist");
        // Check owner of the token ID is the owner and approved
        require(
            crypto4AllNFT.ownerOf(_tokenId) == _msgSender() && crypto4AllNFT.isApproved(_tokenId, address(this)),
    ...
    function confirmOffer(uint256 _tokenId) external payable nonReentrant whenNotPaused {
        ...
        // require(msg.sender.isContract() == false, "NFTMarketplace.confirmOffer: No contracts permitted");
        ...
        require(crypto4AllNFT.isApproved(_tokenId, address(this)), "NFTMarketplace.confirmOffer: offer not approved");
        ...
        (bool sellerTransferSuccess,) = crypto4AllNFT.ownerOf(_tokenId).call{value : bidValue.sub(platformFeeInETH)}("");
        ...
        crypto4AllNFT.safeTransferFrom(crypto4AllNFT.ownerOf(_tokenId), msg.sender, _tokenId);
    ...
    function reclaimETH() external {
        ...
        require(transferSuccess, "NFTMarketplace.reclaimETH: Failed to send eth");
    }
}